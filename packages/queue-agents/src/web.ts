/**
 * queue-agents — web tier (producer).
 *
...
export function createApp(): Hono {
  ...
  app.post('/api/reviews', async (c) => {
    ...
    const id = await createReview(body.prUrl, { source: 'queue-agents', workflow: 'code-review' })
  ...
  app.route('/', createUiRouter('localhost Workshop: Queue Agents'))
...
if (import.meta.url === pathToFileURL(argv[1] ?? '').href) {
  ...
  serve({ fetch: createApp().fetch, port }, (info) => {
    console.info(`[queue-agents:web] listening on http://localhost:${info.port}`)
  })
}
